...
class TransactionSemanticDeduplicationTests {
    ...
    void setUp() throws Exception {
        ...
        // Register User 1
        RegisterRequest request = new RegisterRequest("Semantic", "User", "semantic@example.com", "password123");
    ...
    void shouldBlockSemanticDuplicateWithinWindow() throws Exception {
        TransactionRequest request1 = new TransactionRequest(
                ...
                null, null, null, null, "Deposit 1", UUID.randomUUID().toString()
        );

        TransactionRequest request2 = new TransactionRequest(
                new BigDecimal("100.00"),
                TransactionType.DEPOSIT,
                null, null, null, null, "Deposit 2", UUID.randomUUID().toString() // Different idempotency key!
        );
        ...
        // Second request - Blocked as semantic duplicate
        mockMvc.perform(post("/api/transactions/deposit")
                        .header("Authorization", "Bearer " + token1)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        assertThat(accountRepository.findById(account1.getId()).get().getBalance()).isEqualByComparingTo("100.00");
    }
    ...
    void shouldAllowDifferentAmountWithinWindow() throws Exception {
        ...
        TransactionRequest request2 = new TransactionRequest(
                ...
                TransactionType.DEPOSIT,
                null, null, null, null, "Deposit 2", UUID.randomUUID().toString()
        );
        ...
        assertThat(accountRepository.findById(account1.getId()).get().getBalance()).isEqualByComparingTo("300.00");
    }
}
